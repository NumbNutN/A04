...
for i in range(len(x_train)):
    nlpobj = nlp(x_train[i])
    for sentence in nlpobj.sents:
        sentence_list.append(sentence.text)
        new_label_list.append(y_train[i])
...
for i in range(len(x_test)):
    ...
    for sentence in nlpobj.sents:
        sentence_list.append(sentence.text)
        new_label_list.append(y_test[i])
data = {
    'text':sentence_list,
    'label':[int(label) for label in new_label_list]
}
df = pd.DataFrame(data)
df = df.sort_values(by='label',ascending=True)
df.to_csv("./data/all_content_strenghen_5_18_2_sentence_less_hunlian_test.csv",sep=',',mode='w',header=False,index=False,encoding='utf-8')
