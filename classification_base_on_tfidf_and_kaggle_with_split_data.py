...
# 对文本进行分词
word_list = fet.split_word_from_sentence_array(nlp,train_texts)
print("分词完成", flush=True)

vectorizer = TfidfVectorizer(max_features=3000)
vectorizer = vectorizer.fit_transform(word_list)
train_texts = vectorizer
...
# 对文本进行分词
word_list = fet.split_word_from_sentence_array(nlp,test_texts)
print("分词完成", flush=True)

vectorizer = TfidfVectorizer(max_features=3000)
vectorizer = vectorizer.fit_transform(word_list)
test_texts = vectorizer
...
svc_clf.fit(train_texts,train_labels)
...
y_pred= svc_clf.predict(test_texts)
print(f'The accuracy of train model after tuning is {sm.accuracy_score(train_labels,svc_clf.predict(train_texts))}')
print(f'The accuracy of test model after tuning is {sm.accuracy_score(test_labels,y_pred)}')

import matplotlib.pyplot as plt
plt.figure(figsize=(50,24))
sm.plot_confusion_matrix(svc_clf,test_texts,test_labels)
plt.show()
...
import joblib
joblib.dump(svc_clf,"svc_totally_split_2023_5_13_0.pkl")
...
y_pred = svc_clf.predict_proba(test_texts)
###################################
#          AUX绘图                #
##################################
